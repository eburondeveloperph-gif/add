...
function AudioPlayer({
  ...
  const handlePlay = async () => {
    ...
    try {
      setIsPlaying(true);
      if (!audioRef.current) {
        // Fetch audio from local TTS service
        // Default Chatterbox port is 7860, endpoint is /run/predict usually for Gradio, check docs/code.
        // Assuming a simple GET/POST wrapper or direct call.
        // For Gradio apps, often we need the specific API endpoint.
        // Let's assume there is a proxy or we hit 'http://localhost:7860/api/generate'

        // Construct URL for simple GET if supported, or fetch blob
        const response = await fetch("http://localhost:7860/generate", { // Adjust endpoint based on app.py
          method: "POST",
  ...
  return (
    <Button
      ...
      onClick={handlePlay}
    >
      {isPlaying ? <Pause className="w-4 h-4 mr-2" /> : <Mic className="w-4 h-4 mr-2" />}
...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen relative flex flex-col">
      ...
      {/* Main Content */}
      <section
        className={`relative z-10 grow ${browserSession
          ? "py-12 lg:py-20"
          : "py-8 sm:py-10 flex items-center"
          }`}
      >
        <div
          className={`container mx-auto px-4 ${browserSession ? "max-w-5xl" : "max-w-3xl"
            }`}
        >
              ...
              {!browserSession && (
                <div className="space-y-3 text-center">
                  ...
                  <div className="mt-6 flex justify-center">
                    <div className="relative w-full max-w-md aspect-video rounded-2xl border border-slate-800 bg-linear-to-br from-slate-900 via-slate-950 to-black shadow-[0_0_40px_rgba(0,0,0,0.6)] overflow-hidden">
                          ...
                          {showServerSettings && (
                            <div className="mt-3 grid gap-2 sm:grid-cols-2">
                              <button
                                ...
                                className={`rounded-md border p-3 text-left transition ${serverTarget === "vps"
                              ...
                              <button
                                ...
                                className={`rounded-md border p-3 text-left transition ${serverTarget === "cloud-eu"
                                ...
                                <div className="space-y-3">
                                  ...
                                  {result.response && (
                                    <div>
                                      ...
                                      {/* Auto-play TTS for the most recent result if it matches the current accumulation */}
                                      <AudioPlayer
